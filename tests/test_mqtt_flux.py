"""
Transmission Node Test - MQTT ENABLED
======================================

Test transmission node with real MQTT.
"""
...
def test_transmission_node():
    ...
    # Calculate stats from log (no more transmission_stats in return)
    log = result['transmission_log']
    total = len(log)
    success = sum(1 for e in log if e.get('success'))
    failed = total - success
    success_rate = success / total if total > 0 else 0
    total_bytes = sum(e.get('payload_size_bytes', 0) for e in log)
    
    print(f"\n📊 Test Results:")
    print(f"Messages transmitted: {total}")
    ...
    for log_entry in log:
        ...
        topic = log_entry.get('topic', log_entry.get('recipient_id'))
        print(f"  {status} {log_entry['message_id']} → {topic}")
